...
export type TemplateKey = 'DOWNLOAD_WELCOME' | 'DOWNLOAD_LINK' | 'NEWSLETTER_CONFIRMATION';
...
export const FALLBACK_TEMPLATES: Record<TemplateKey, ActiveTemplate> = {
  DOWNLOAD_WELCOME: {
    id: null,
    key: 'DOWNLOAD_WELCOME',
    version: 2,
    subjectTemplate: 'Rejestracja pobrania {{productName}} została przyjęta',
    textBodyTemplate: `Dzień dobry,
...
<html lang="pl">
  ...
  DOWNLOAD_LINK: {
    ...
    key: 'DOWNLOAD_LINK',
    version: 2,
    subjectTemplate: 'Pobierz {{productName}}',
    textBodyTemplate: `Dzień dobry,
...
function interpolateTemplate(
  ...
  return template.replace(TEMPLATE_VARIABLE_PATTERN, (_, key: string) => transformValue(values[key] ?? ''));
}
...
export function interpolateTextTemplate(template: string, values: Record<string, string>) {
  if (template.includes('\\n')) {
    throw new Error('Transactional email text templates must use real newline characters, not literal \\n sequences.');
  }

  return interpolateTemplate(template, values, (value) => value);
}

export function interpolateHtmlTemplate(template: string, values: Record<string, string>) {
  return interpolateTemplate(template, values, escapeHtml);
}

export function buildPlainTextHtmlDocument(textBody: string) {
  return `<!doctype html><html lang="pl"><body><pre style="white-space:pre-wrap;font-family:Arial,sans-serif;">${escapeHtml(textBody)}</pre></body></html>`;
}

export function redactRenderedValue(content: string, rawValue: string, replacement: string) {
  if (!rawValue) {
    return content;
  }

  return content.replaceAll(rawValue, replacement).replaceAll(escapeHtml(rawValue), replacement);
}
